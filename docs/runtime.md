...
The [`Pop` modifier](Every time a fan-out happens, the existing `Fan-out` field from the input is moved to a nested `OuterLoop` field. ) removes the most recent loop and makes the first `OuterLoop` the `Fan-out` field in the input payload.
