...
def main():
	...
	communication_latency = np.array([(second_function_tsp[i][0] - first_function_tsp[i][1]) for i in range(len(first_function_tsp))])

	fig, axs = plt.subplots()
	axs.set_title('1st end to 2nd start')
	axs.scatter(np.array([i for i in range(len(communication_latency))]), communication_latency)
	axs.text(0.4,0.9, '$\mu=${:.2f}, $\sigma=${:.2f}'.format(np.mean(communication_latency), np.std(communication_latency)), transform=axs.transAxes)
